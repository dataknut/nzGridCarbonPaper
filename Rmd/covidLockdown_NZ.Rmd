 ...
 * See [report's edit history](https://github.com/CfSOtago/gridCarbon/commits/master/dataAnalysis/)
...
p <- ggplot2::ggplot(plotDT[obsDate > as.Date("2020-01-01")], 
  ...
  geom_vline(xintercept = repoParams$NZLevel4StartDate) +
  labs(x = dateLab,
...
# p <- addLockdownRect(p, 
#                 from = repoParams$UKlockDownStartDate, 
#                 to = repoParams$UKlockDownEndDate, 
#                 yMin = yMin, yMax = yMax)
p
ggplot2::ggsave(filename = "NZ_MWhFuelsTrendsStacked.png", 
...
ggplot2::ggsave( filename = "NZ_MWhFuelsTrendsNonStacked.png", 
                path = localParams$plotPath ,
                height = 10, width = 10, units = "in",
                p)
```
...
```{r kWhBySeason2019}
allGridDT <- gridCarbon::addSeason(allGridDT, h = "S", dateVar =  "rDate")
...
ggplot2::ggsave(filename = "NZ_TotalElectricityGeneration.jpeg", 
                myPlotTot, path = localParams$plotPath,
                dpi = 600, width = 13.333, height = 7.5, units = "in")
```
...
plotDT <- plotDT[, time := hms::as_hms(rDateTimeNZT)]
